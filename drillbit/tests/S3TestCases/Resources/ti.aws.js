...
var _sessionOBJ = {
	...
	"iphone" === Ti.Platform.osname ? c.open(this.verb, this.endpoint + "?" + payload) : c.open(this.verb, this.endpoint);
	...
	"iphone" === Ti.Platform.osname ? c.send() : c.send(payload)
	...
	var h = this.method;
	c.onload = function() {
		"GET" == this.connectionType || "POST" == this.connectionType || "uploadPartCopy" == h ? "getObjectTorrent" === h || "getObject" === h || "getBucketPolicy" === h ? d && (c.getResponseHeader("ETag") && Titanium.API.info("ETag:" + c.getResponseHeader("ETag")), d(this.responseText)) : d && d(_sessionOBJ.xmlToJSON.toJSON(this.responseText, !0)) : d && (c.getResponseHeader("ETag") && Titanium.API.info("ETag:" + c.getResponseHeader("ETag")), d(this.responseText))
	};
	c.onerror = function() {
		if(b) {
			var a = _sessionOBJ.xmlToJSON.toJSON(this.responseText, !0);
